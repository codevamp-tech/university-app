...
import { createShopGigAPI, uploadAvatarAPI } from '../../data/apiService';

const AddGigScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!imageUri) {
      Alert.alert('Image Required', 'Please add a cover photo for your gig.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Success!', 'Your gig is now live! Students can now see and purchase your item.', [
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border, paddingTop: insets.top }]}>
        <View style={styles.headerInner}>
          ...
          <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Offer a Gig</Text>
          <View style={{ width: 24 }} />
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.formGroup}>
          <Text style={[styles.label, { color: colors.textPrimary }]}>Gig Title *</Text>
          <TextInput
            style={[styles.input, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
            placeholder="e.g. Logo Design, Website Development, Math Tutoring"
        ...
        <View style={styles.formGroup}>
          <Text style={[styles.label, { color: colors.textPrimary }]}>Rate / Price (₹) *</Text>
          <TextInput
            style={[styles.input, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
            placeholder="e.g. 500"
            placeholderTextColor={colors.textMuted}
        ...
        <View style={styles.formGroup}>
          <Text style={[styles.label, { color: colors.textPrimary }]}>Gig Details & Description *</Text>
          <TextInput
            style={[styles.input, styles.textArea, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
            placeholder="Describe what services you will provide, your experience, timeframe, etc..."
            placeholderTextColor={colors.textMuted}
            value={description}
        ...
        >
          ...
          >
            {isSubmitting ? (
              ...
              <Text style={styles.submitText}>Offer Gig</Text>
            )}
          </LinearGradient>
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={isSubmitting} transparent animationType="fade">
        <View style={styles.loaderOverlay}>
          <View style={[styles.loaderBox, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.loaderText, { color: colors.textPrimary }]}>Posting Gig...</Text>
          </View>
        </View>
      </Modal>
    </KeyboardAvoidingView>
