...
import { createShopRequestAPI, uploadAvatarAPI } from '../../data/apiService';
...
const AddRequestScreen = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!imageUri) {
      Alert.alert('Image Required', 'Please add a cover photo for your request.');
      return;
    }
    ...
    try {
      ...
      const response = await createShopRequestAPI(accessToken, {
        title: title.trim(),
      ...
      DeviceEventEmitter.emit('newRequestAdded', {
        id: 'local_' + Date.now(),
        title: title.trim(),
        description: description.trim(),
        price: parseFloat(price),
      ...
      Alert.alert('Success!', 'Your request is now live on the Hub!', [
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border, paddingTop: insets.top }]}>
        <View style={styles.headerInner}>
          ...
          <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Post a Request</Text>
          <View style={{ width: 24 }} />
        </View>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.formGroup}>
          <Text style={[styles.label, { color: colors.textPrimary }]}>What do you need? (Request Title) *</Text>
          <TextInput
            style={[styles.input, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
            placeholder="e.g. Need help moving boxes, Tutoring for Exam"
        ...
        <View style={styles.formGroup}>
              <Text style={[styles.label, { color: colors.textSecondary }]}>Willing to Pay (Budget) *</Text>
          <TextInput
            style={[styles.input, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
            placeholder="e.g. 500"
            placeholderTextColor={colors.textMuted}
        ...
        <View style={styles.formGroup}>
          <Text style={[styles.label, { color: colors.textPrimary }]}>Request Details & Requirements *</Text>
          <TextInput
      ...
      <Modal visible={isSubmitting} transparent animationType="fade">
        <View style={styles.loaderOverlay}>
          <View style={[styles.loaderBox, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.loaderText, { color: colors.textPrimary }]}>Posting Request...</Text>
          </View>
        </View>
      </Modal>
    </KeyboardAvoidingView>
